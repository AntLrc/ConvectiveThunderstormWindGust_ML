...
def loss_and_CRPS(state, params, batch, batch_size, total_len, n_clusters,
...
def mae(state, params, batch, batch_size, total_len, n_clusters,
        regularisation = None, alpha = 0.01):
    x_s, x_t, y_true = batch
    y_pred = state.apply_fn(params, x_s, x_t)
...
def createTrainState(model, rng, learning_rate, batch_size, features, stationwise = False, n_stations = 0):
    if stationwise:
        params = model.init(rng,jnp.ones((batch_size, n_stations, features)), jnp.ones((batch_size, 4)), jnp.ones((batch_size, 4)))
    else:
...
def train_step(state, x_s, x_t, y_true,
               ...
               regularisation = None, alpha = 0.01, target = 0):
    if target == 0:
        def loss_fn(state, params, batch, batch_size, total_len, n_clusters,
                        regularisation = None, alpha = 0.01, dropout_train_key = None):
            x_s, x_t, y_true = batch
            ...
            return loss + (regularisation == "l2")*sum(l2_loss(w, alpha=alpha) for w in jax.tree.leaves(params)) + (regularisation == "l1")*sum(l1_loss(w, alpha=alpha) for w in jax.tree.leaves(params))
    else:
        def loss_fn(state, params, batch, batch_size, total_len, n_clusters,
                        regularisation = None, alpha = 0.01, dropout_train_key = None):
            x_s, x_t, y_true = batch
    ...
    loss, grads = jax.value_and_grad(loss_fn, argnums = 1)(state, state.params, (x_s, x_t, y_true), batch_size, total_len, n_clusters, regularisation, alpha, dropout_train_key)
    state = state.apply_gradients(grads = grads)
    return state 

def train_loop(state,
               train_features_s, train_features_t, train_labels,
               val_features_s, val_features_t, val_labels,
               batch_size, epochs, total_len, n_clusters, rngshuffle,
               regularisation = None, alpha = 0.01, n_best_states = 3, target = 0, early_stopping = 6):
    train_loss_arr = []
    train_CRPS_arr = []
    val_loss_arr = []
    val_CRPS_arr = []
    best_params_with_scores = []
    metrics_fn = loss_and_CRPS if target == 0 else mae # Convention: 0 for GEV, 1 for values
    ...
    for epoch in range(epochs):
        ...
        for x_s, x_t, y_true in createBatches(train_features_s, train_features_t, train_labels, batch_size, rngshuffle):
            ...
            if percent >= 5:
                ...
                percent -= 5
            state = train_step(state, x_s, x_t, y_true,
                                            batch_size, total_len, n_clusters,
                                            regularisation=regularisation, alpha=alpha, target=target)
        print("")
        ...
        count = 0
        for x_s, x_t, y_true in createBatches(train_features_s, train_features_t, train_labels, batch_size, rngshuffle):
            tmp_crps, tmp_loss = metrics_fn(state, state.params, (x_s, x_t, y_true), batch_size, total_len, n_clusters, regularisation, alpha)
            train_CRPS += tmp_crps
            train_loss += tmp_loss
            count += 1
        ...
        if early_stopping > 0 and epoch > early_stopping:
            if all([val_CRPS_arr[-i] > val_CRPS_arr[-i-1] for i in range(1, early_stopping + 1)]):
                break
        
    return best_params_with_scores, train_loss_arr, val_loss_arr, train_CRPS_arr, val_CRPS_arr