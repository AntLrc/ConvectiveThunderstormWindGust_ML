...
def crps_arr(obs, fcst, nfold):
    ...
    for i in range(len(obs)):
        CRPS[i] = compute_ecdf_crps(obs[i], fcst[i])
...
def crps_arr_climatology(obs, climatology, nfold):
    """
    The function outputs the CRPS of the baseline.
    ...
    for i in range(len(obs)):
        print("Cluster: ", i)
        CRPS[i] = compute_ecdf_crps_climatology(obs[i], climatology[i])
    return np.repeat(np.expand_dims(CRPS, axis=0), nfold, axis=0)
...
def pointwise_baseline(obs_da, fcst_da, tarray, year, clusters):
    ...
    hour = (
        np.angle([complex(tarray[i, 1], tarray[i, 2]) for i in range(len(tarray))])
        * 12
        / np.pi
    )
    hour += 24 * (hour < 0)
