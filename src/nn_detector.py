...
class DenseNet(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = nn.Conv(
            c_hidden, kernel_size=(3, 3), kernel_init=nn.initializers.kaiming_normal()
        )(x)
...
def compute_metrics(logits, labels, threshold=0.5):
    ...
    accuracy = jnp.mean(
        jnp.all(jnp.where(nn.sigmoid(logits) >= threshold, 1.0, 0.0) == labels, axis=1)
    )
...
@jax.jit
def train_step(state, x_s, x_t, y, alpha=0.0, threshold=0.5):
...
@jax.jit
def eval_step(state, params, x_s, x_t, y, threshold=0.5):
    logits = state.apply_fn(
        {"params": params, "batch_stats": state.batch_stats}, x_s, x_t
    )
    return compute_metrics(logits, y, threshold=threshold)


def train_epoch(state, train_ds, batch_size, epoch, rng, alpha=0.0, threshold=0.5):
...
def eval_model(state, params, test_ds, threshold=0.5):
    x_s = test_ds["image"]
    x_t = test_ds["time"]
    y = test_ds["label"]
...
def __main__():
    ...
    parser.add_argument(
        "--folder",
        type=str,
        help="Path to the dir containing the datasets and to which will be plotted the results",
    )
    parser.add_argument("--epochs", type=int, default=10, help="Number of epochs")
    parser.add_argument(
        "--learning-rate", type=float, default=0.0001, help="Learning rate"
    )
    parser.add_argument("--penalty", type=float, default=0.0, help="L2 penalty")
    parser.add_argument(
    ...
    with open(os.path.join(folder, "command.txt"), "w") as f:
        f.write(
            f"run /work/FAC/FGSE/IDYST/tbeucler/downscaling/alecler1/ConvectiveThunderstormWindGust_ML/src/nn_detector.py --folder {folder} --epochs {args.epochs} --learning-rate {args.learning_rate} --penalty {args.penalty} --threshold {args.threshold} --model {args.model}"
        )
    ...
    for i, ds in enumerate([train_ds, val_ds, test_ds]):
        ...
        axs[i].set_title(
            f"Number of storms in the dataset {['train', 'val', 'test'][i]}"
        )
    ...
    if args.model.lower() == "cnn":
        ...
        model = Detector(
            ...
            fcn=FCN(features=64, use_bias=True, layers=1),
    ...
    logits = model.apply(
        {"params": state.params, "batch_stats": state.batch_stats}, x_s, x_t
    )
    logits = jax.device_get(logits)
    res = nn.sigmoid(logits)
    res = jnp.where(res >= args.threshold, 1.0, 0.0)
