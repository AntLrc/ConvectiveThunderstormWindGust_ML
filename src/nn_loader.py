...
class Experiment:
    ...
    def __init__(self, experiment_file):
        ...
        default_model_kwargs = {'batch_size': 64,
            'rng':{
                'init': jax.random.key(0),
                'shuffle': jax.random.key(1)
            },
            'epochs':  50,
            'learning_rate': 1e-4,
            'regularisation': None,
    ...
    def load_mean_std(self):
        ...
        for i, file in enumerate(self.files['train']['inputs']):
            ...
            if self.filter['storm_part']['train'] is not None:
                ...
                if (ratio == 1) or len(storm_dates) >= len(nostorm_dates)*ratio/(1-ratio):
                    storm_dates = storm_dates[jax.random.permutation(rng, len(storm_dates))[:int(len(nostorm_dates)*ratio/(1-ratio) if ratio != 1. else -1)]]
                    loc_dates = storm_dates.union(nostorm_dates)
                else:
                    nostorm_dates = nostorm_dates[jax.random.permutation(rng, len(nostorm_dates))[:int(len(storm_dates)*(1-ratio)/ratio if ratio != 0. else -1)]]
                    loc_dates = storm_dates.union(nostorm_dates)
    ...
    def create_inputs(self):
        ...
        for file, labels, fold in zip(self.files['train']['inputs'], self.files['train']['labels'], range(len(self.files['train']['inputs']))):
            ...
            if self.model_kwargs['time_encoding'] == 'sinusoidal':
                npinputs_t = np.array([[[(time.day_of_year - 242)/107, # Encoding of day of year between -1 and +1
                                    np.cos(time.hour*np.pi/12),
                                    np.sin(time.hour*np.pi/12),
                                    lt/72] for time in loc_dates] for lt in inputs.lead_time],
                                 dtype = np.float32).reshape(-1,4)
                npstorms = np.repeat(np.expand_dims(storm_presence, axis = 0),
                                        len(inputs.lead_time), axis = 0).reshape(-1)
            ...
            for ivar in range(len(self.features)):
                ...
                sh = tmp.shape
                if self.filter['2d']:
                    tmp = tmp.reshape((sh[0]*sh[1], sh[2], sh[3])) # Shape of (lead_time*time, lat, lon)
                else:
                    tmp = tmp.reshape((sh[0]*sh[1], sh[2])) # Shape of (lead_time*time, station)
                tmp = (tmp - self.Data['mean'][ivar])/self.Data['std'][ivar] # Normalizing
                tmp = np.expand_dims(tmp, axis = -1)
                npinputs_s = tmp if npinputs_s is None else np.concatenate([npinputs_s, tmp], axis = -1)
            # Labels
            ...
            for i in range(self.clusters['n']):
                ...
                nplabels[i] = labtmp
            # Saving
            with open(os.path.join(self.folders['scratch']['folder'], f"fold_{fold}.pkl"), 'wb') as f:
                pickle.dump((npinputs_s, npinputs_t, nplabels), f)
            with open(os.path.join(self.folders['scratch']['folder'], f"storms_{fold}.pkl"), 'wb') as f:
        ...
        for file, labels in zip(self.files['test']['inputs'], self.files['test']['labels']):
            ...
            if self.filter['storm_part']['test'] is not None:
                ...
                nostorm_dates = input_dates.difference(self.filter['dates'])
                if (ratio != 1) and len(storm_dates) >= len(nostorm_dates)*ratio/(1-ratio):
                    storm_dates = storm_dates[jax.random.permutation(rng, len(storm_dates))[:int(len(nostorm_dates)*ratio/(1-ratio))]]
                    loc_dates = storm_dates.union(nostorm_dates)
                else:
                    nostorm_dates = nostorm_dates[jax.random.permutation(rng, len(nostorm_dates))[:int(len(storm_dates)*(1-ratio)/ratio)]]
                    loc_dates = storm_dates.union(nostorm_dates)
                loc_dates = loc_dates.sort_values()
            # Encoding of time
            if self.model_kwargs['time_encoding'] == 'sinusoidal':
        ...
        with open(os.path.join(self.folders['scratch']['folder'], "test_set.pkl"), 'wb') as f:
            pickle.dump((npinputs_s, npinputs_t, nplabels), f)
        print("Done.", flush = True)
    ...
    def load_data(self):
        ...
        for ifold in range(self.model_kwargs['n_folds']):
            with open(os.path.join(self.folders['scratch']['folder'], f"fold_{ifold}.pkl"), 'rb') as f:
                s, t, l = pickle.load(f)
            with open(os.path.join(self.folders['scratch']['folder'], f"storms_{ifold}.pkl"), 'rb') as f:
                storms = pickle.load(f)
            self.train_s.append(s)
            self.train_t.append(t)
            self.train_l.append(l)
            self.train_storms.append(storms)
        with open(os.path.join(self.folders['scratch']['folder'], 'test_set.pkl'), 'rb') as f:
    ...
    def NN_(self):
        NNs = {
            'Dense_NN':Dense_NN,
            'DeepFCN':DeepFCN,
            'Identity_NN':Identity_NN,
            'Killed_NN':Killed_NN,
            'Conv_NN':Conv_NN,
            'ConvNeXt_Block':ConvNeXt_Block,
            'ConvNeXt_Blocks_NN':ConvNeXt_Blocks_NN,
            'ConvNeXt_NN':ConvNeXt_NN,
            'AttentionBlock':AttentionBlock,
            'VisionTransformer':VisionTransformer,
            'AddTrainableXi':AddTrainableXi,
            'SimpleBaseline':SimpleBaseline,
            'DDNN_GEV':DDNN_GEV,
            'Discrete_NN':Discrete_NN
        }        
        if not self.nn_kwargs['spatial'] is None:
            Spatial_NN = NNs[self.nn_kwargs['spatial']['name']](**self.nn_kwargs['spatial']['kwargs'])
        if not self.nn_kwargs['temporal'] is None:
            Temporal_NN = NNs[self.nn_kwargs['temporal']['name']](**self.nn_kwargs['temporal']['kwargs'])
        if not self.nn_kwargs['distributional'] is None:
            DDNN = NNs[self.nn_kwargs['distributional']['name']](**self.nn_kwargs['distributional']['kwargs'])
        return AlpTh_NN(
    ...
    def run(self, preComputed = False, fold = None):
        # Create data if needed
        if not (all([os.path.exists(os.path.join(self.folders['scratch']['folder'], f"fold_{i}.pkl")) for i in range(self.model_kwargs['n_folds'])]) and os.path.exists(os.path.join(self.folders['scratch']['folder'], 'test_set.pkl'))):
        ...
        self.load_data()
        # Visualise data
        if not all([os.path.exists(os.path.join(self.folders['plot']['folder'], file + 'Distribution.png')) for file in ['Labels', 'Features']]):
            print("Visualising data...", flush = True)
            pass
        # Train self.model_kwargs['n_folds'] models
        if not preComputed:
            if not os.path.exists(os.path.join(self.folders['plot']['folder'], 'params.pkl')):
                params = [None]*self.model_kwargs['n_folds']
            else:
                with open(os.path.join(self.folders['plot']['folder'], 'params.pkl'), 'rb') as f:
                    params = pickle.load(f)
            if not os.path.exists(os.path.join(self.folders['plot']['folder'], 'losses.pkl')):
                losses = [None]*self.model_kwargs['n_folds']
            else:
                with open(os.path.join(self.folders['plot']['folder'], 'losses.pkl'), 'rb') as f:
                    losses = pickle.load(f)
            if not os.path.exists(os.path.join(self.folders['plot']['folder'], 'metrics.pkl')):
                metrics = [None]*self.model_kwargs['n_folds']
            else:
                with open(os.path.join(self.folders['plot']['folder'], 'metrics.pkl'), 'rb') as f:
                    metrics = pickle.load(f)
            for ifold in range(self.model_kwargs['n_folds']):
                if fold is not None and ifold != fold:
                    continue
                print(f"Training model with fold {ifold} as validation set...", flush = True)
                model_state = createTrainState(self.NN,
                ...
                # Get data
                train_s = jnp.array(np.concatenate([self.train_s[i] for i in range(self.model_kwargs['n_folds']) if i != ifold], axis = 0))
                train_t = jnp.array(np.concatenate([self.train_t[i] for i in range(self.model_kwargs['n_folds']) if i != ifold], axis = 0))
                train_l = tuple([jnp.array(np.concatenate([self.train_l[i][j] for i in range(self.model_kwargs['n_folds']) if i != ifold], axis = 0)) for j in range(self.clusters['n'])])
                val_s = jnp.array(self.train_s[ifold][self.train_storms[ifold] == 1])
                val_t = jnp.array(self.train_t[ifold][self.train_storms[ifold] == 1])
                val_l = tuple([jnp.array(self.train_l[ifold][j][self.train_storms[ifold] == 1]) for j in range(self.clusters['n'])])
                #Print shapes
                print(f"Train set shapes:\n - shape of spatial inputs: {train_s.shape}\n - shape of temporal inputs: {train_t.shape}\n - shape of labels: {train_l[0].shape}")
                ...
                # Train model
                best_params_with_scores, train_loss, val_loss, train_CRPS, val_CRPS = train_loop(model_state,
                                                train_s, train_t, train_l,
                                                val_s, val_t, val_l,
                                                self.model_kwargs['batch_size'], self.model_kwargs['epochs'], len(self.clusters['stations']), self.clusters['n'],
                                                self.model_kwargs['rng']['shuffle'], regularisation=self.model_kwargs['regularisation'], alpha=self.model_kwargs['alpha'],
                                                n_best_states=self.model_kwargs['n_best_states'], target=self.target, early_stopping=self.model_kwargs['early_stopping']
                                                )
                output_params = jax.tree.map(lambda *x: jnp.stack(x).mean(axis = 0), *map(lambda x: x[0], best_params_with_scores))
                params[ifold] = output_params
                losses[ifold] = {'train_loss':train_loss, 'val_loss':val_loss, 'train_CRPS':train_CRPS, 'val_CRPS':val_CRPS}
                output_state = model_state.replace(params = output_params)
                # Evaluate model
                output_CRPS = 0
                count = 0
                metrics_fn = loss_and_CRPS if self.model_kwargs['target'] == 'GEV' else mae
                for x_s, x_t, y_true in createBatches(val_s, val_t, val_l, self.model_kwargs['batch_size'], self.model_kwargs['rng']['shuffle']):
                    tmp_crps, _ = metrics_fn(output_state, output_state.params, (x_s, x_t, y_true),
                                            self.model_kwargs['batch_size'], len(self.clusters['stations']), self.clusters['n'],
                                            self.model_kwargs['regularisation'], self.model_kwargs['alpha'])
                    output_CRPS += tmp_crps
                    count += 1
                metrics[ifold] = output_CRPS/count
                # Save results at each fold
                with open(os.path.join(self.folders['plot']['folder'], 'params.pkl'), 'wb') as f:
                    pickle.dump(params, f)
                with open(os.path.join(self.folders['plot']['folder'], 'losses.pkl'), 'wb') as f:
                    pickle.dump(losses, f)
                with open(os.path.join(self.folders['plot']['folder'], 'metrics.pkl'), 'wb') as f:
    ...
    def __str__(self):
        ...
        erls = "After " + str(self.model_kwargs['early_stopping']) + " epochs" if self.model_kwargs['early_stopping'] != 0 else "No early stopping"
        result += f"\nEarly stopping: {erls}"
    ...
    class Diagnostics:
        ...
        def allCRPS(self, save = False):
            ...
            CRPS = np.zeros((self.experiment.model_kwargs['n_folds'],
                             ...
                             t.shape[0]))
            with open(os.path.join(self.experiment.folders['plot']['folder'], 'params.pkl'), 'rb') as f:
                paramlist = pickle.load(f)
            gevparams = None
            for i, params in enumerate(paramlist):
                # Compute outputs
                for ibatch in range(len(t)//100 + 1):
                    S = s[100*ibatch:100*(ibatch+1)]
                    T = t[100*ibatch:100*(ibatch+1)]
                    L = tuple([l[j][100*ibatch:100*(ibatch+1)] for j in range(self.experiment.clusters['n'])])
                    if len(S) == 0:
                        continue
                    gevparams = self.experiment.NN.apply(params, S, T)
                    mus, sigmas, xis = jnp.split(gevparams, 3, axis = 1)
                    for icluster in range(self.experiment.clusters['n']):
                        crps = gevCRPS(
                            jnp.repeat(jnp.expand_dims(mus[:,icluster], axis = 1), L[icluster].shape[1], axis = 1),
                            jnp.repeat(jnp.expand_dims(sigmas[:,icluster], axis = 1), L[icluster].shape[1], axis = 1),
                            jnp.repeat(jnp.expand_dims(xis[:,icluster], axis = 1), L[icluster].shape[1], axis = 1),
                            L[icluster]
                        ).mean(axis = 1) # Mean on stations
    ...
    class Saver:
        ...
        def experimentfile(self):
            experiment_dict = {
                ...
                'filter':{k:self.experiment.filter[k] for k in ['lead_times', 'storm_part', '2d']},
    ...
    class Plotter:
        ...
        def pithist(self):
            assert os.path.exists(os.path.join(self.experiment.folders['plot']['folder'], 'best_states.pkl')), "Run the experiment first."
            if not hasattr(self.experiment, 'test_s'):
                self.experiment.load_data()
            with open(os.path.join(self.experiment.folders['plot']['folder'], 'best_states.pkl'), 'rb') as f:
            ...
            print(len(set_l[0]))
            param_pred = jnp.concatenate([self.experiment.NN.apply(output_params, set_s[ntimes*i:ntimes*(i+1)], set_t[ntimes*i:ntimes*(i+1)]) for i in range(100)], axis = 0)
            print(len(param_pred))
            PIT_histogram(tuple(map(lambda x:x[:ntimes*100], set_l)),
                        param_pred,
                        os.path.join(self.experiment.folders['plot']['folder'], "PIT_histogram.png"),
                        title = "PIT histogram", leadtime = None)
            
            return gevCRPSLoss(param_pred, tuple(map(lambda x:x[:len(set_s)//100*100], set_l)), len(self.experiment.clusters['stations']), len(set_s)//100*100, self.experiment.clusters['n'])
        
        def casestudy(self, date, lead_time):
            assert self.experiment.filter['2d'], "Case study only available for 2D data."
            ...
            x_t = np.array([[[(date.day_of_year - 242)/107, # Encoding of day of year between -1 and +1
                                        np.cos(date.hour*np.pi/12),
                                        np.sin(date.hour*np.pi/12),
                                        lt/72] for lt in ds.lead_time]],
                                    dtype = np.float32).reshape(-1,4)
            x_s = None
            ...
            for i in range(self.experiment.clusters['n']):
                ...
                labtmp = np.repeat(np.expand_dims(labtmp, axis = 0), len(ds.lead_time), axis = 0)
                nplabels[i] = labtmp
            windgusts = tuple([jnp.array(nplabels[i], dtype = jnp.float32) for i in range(self.experiment.clusters['n'])])
            
            
            gev_params = list(map(lambda param : model.apply(param, x_s, x_t), param_set))
            gev_params = np.array(gev_params) # Shape : (n_fold, n_lead_time, n_clusters*3)
            plot_params = gev_params[:,ilt[lead_time],:] #Shape : (n_fold, n_clusters*3)
            
            mus = plot_params[:,:5]
            sigmas = plot_params[:,5:10]
            xis = plot_params[:,10:]
            
            plot_params = plot_params.reshape(self.experiment.model_kwargs['n_folds'], 3, self.experiment.clusters['n']).transpose(0,2,1)
            
            # Compute CRPS for each cluster
            CRPSs = np.array([[
                gevCRPS(jnp.expand_dims(mus[[ifold],[icluster]], axis = 1).repeat(len(self.experiment.clusters['groups'][icluster]), axis = -1),
                        jnp.expand_dims(sigmas[[ifold],[icluster]], axis = 1).repeat(len(self.experiment.clusters['groups'][icluster]), axis = -1),
                        jnp.expand_dims(xis[[ifold],[icluster]], axis = 1).repeat(len(self.experiment.clusters['groups'][icluster]), axis = -1),
                        windgusts[icluster]).mean()
                for icluster in range(self.experiment.clusters['n'])]
            for ifold in range(self.experiment.model_kwargs['n_folds'])
            ])
            lead_times = [k for k in ilt.keys()]
            
            stormplot(stations, self.experiment.filter['storms'], plot_params, tuple(map(lambda x : x[ilt[lead_time]], windgusts)),
        ...
        def losses(self, exp_losses):
            fig, ax = plt.subplots(2, self.experiment.model_kwargs['n_folds'], figsize = (5*self.experiment.model_kwargs['n_folds'], 10),
                                   sharex = True, sharey = 'row', subplot_kw = {'xlabel':'Epochs'})
            ...
            plt.savefig(os.path.join(self.experiment.folders['plot']['folder'], 'Losses.png'))
        
        def ltCRPS(self, keep = False):
            """
            Plots the variation of the CRPS with the lead time.
            """
            if not os.path.exists(os.path.join(self.experiment.folders['plot']['folder'], 'params.pkl')):
                raise ValueError("Run the experiment first.")
            with open(os.path.join(self.experiment.folders['plot']['folder'], 'params.pkl'), 'rb') as f:
                paramlist = pickle.load(f)
            CRPS = np.zeros((self.experiment.model_kwargs['n_folds'], len(self.experiment.filter['lead_times'])))
            with open(os.path.join(self.experiment.folders['scratch']['folder'], f"test_set.pkl"), 'rb') as f:
                s, t, l = pickle.load(f)
            for i, params in enumerate(paramlist):
                for j, lt in enumerate(self.experiment.filter['lead_times']):
                    print(lt)
                    S = jnp.array(s[t[:,3] == lt/72])
                    T = jnp.array(t[t[:,3] == lt/72])
                    L = tuple([jnp.array(l[i][t[:,3] == lt/72]) for i in range(self.experiment.clusters['n'])])
                    count = 0
                    for x_s, x_t, y_true in createBatches(S, T, L, self.experiment.model_kwargs['batch_size'], self.experiment.model_kwargs['rng']['shuffle']):
                        gevparams = self.experiment.NN.apply(params, x_s, x_t)
                        if self.experiment.model_kwargs['target'] == 'GEV':
                            CRPS[i,j] += gevCRPSLoss(gevparams, y_true, len(self.experiment.clusters['stations']), self.experiment.model_kwargs['batch_size'], self.experiment.clusters['n']).mean()
                        else:
                            CRPS[i,j] += mae(gevparams, y_true, self.experiment.model_kwargs['batch_size'], len(self.experiment.clusters['stations']), self.experiment.clusters['n'])
                        count += 1
                    CRPS[i,j] /= count
        ...
        def clusterCRPS(self, cluster, date = None):
            ...
            with open(os.path.join(self.experiment.folders['plot']['folder'], 'params.pkl'), 'rb') as f:
                paramlist = pickle.load(f)
            CRPS = np.zeros((self.experiment.model_kwargs['n_folds'], len(self.experiment.filter['lead_times'])))
            ds = xr.open_dataset(self.experiment.files['test']['inputs'][0], engine = "netcdf4")
            if date is not None:
                ...
                tmp_t = np.array([[[(time.day_of_year - 242)/107, # Encoding of day of year between -1 and +1
                                    np.cos(time.hour*np.pi/12),
                                    np.sin(time.hour*np.pi/12),
                                    lt/72] for time in input_time] for lt in self.experiment.filter['lead_times']],
                                 dtype = np.float32).reshape(-1,4)
            with open(os.path.join(self.experiment.folders['scratch']['folder'], f"test_set.pkl"), 'rb') as f:
                s, t, l = pickle.load(f)
            index = np.array([any([np.all(np.where(ti == titmp, True, False)) for titmp in tmp_t]) for ti in t])
            s = s[index]
            ...
            for i, params in enumerate(paramlist):
                for j, lt in enumerate(self.experiment.filter['lead_times']):
                    ...
                    T = jnp.array(t[t[:,3] == lt/72])
                    L = tuple([jnp.array(l[i][t[:,3] == lt/72]) for i in range(self.experiment.clusters['n'])])
                    outputs = self.experiment.NN.apply(params, S, T)
                    mus, sigmas, xis = jnp.split(outputs, 3, axis = 1)
                    CRPS[i,j] = gevCRPS(jnp.repeat(jnp.expand_dims(mus[:,cluster], axis = 1), L[cluster].shape[-1], axis = 1),
                                        jnp.repeat(jnp.expand_dims(sigmas[:,cluster], axis = 1), L[cluster].shape[-1], axis = 1),
                                        jnp.repeat(jnp.expand_dims(xis[:,cluster], axis = 1), L[cluster].shape[-1], axis = 1),
                                                   L[cluster]).mean()
            Metricsevolution({"CRPS":CRPS}, self.experiment.filter['lead_times'], os.path.join(self.experiment.folders['plot']['folder'], f'CRPS_cluster_{cluster}{"_" + date if date is not None else None}.png'))
            return CRPS
        ...
        def returnlevel(self, returnlevels):
            ...
            with open(os.path.join(self.experiment.folders['plot']['folder'], 'params.pkl'), 'rb') as f:
                paramlist = pickle.load(f)
            RLmodel = np.zeros((self.experiment.model_kwargs['n_folds'], len(self.experiment.filter['lead_times']), len(returnlevels)))
            for i, params in enumerate(paramlist):
                with open(os.path.join(self.experiment.folders['scratch']['folder'], f"fold_{i}.pkl"), 'rb') as f:
                    s, t, l = pickle.load(f)
                with open(os.path.join(self.experiment.folders['scratch']['folder'], f"storms_{i}.pkl"), 'rb') as f:
                    storms = pickle.load(f)
                s = s[storms == 1]
                t = t[storms == 1]
                ...
                for j, lt in enumerate(self.experiment.filter['lead_times']):
                    ...
                    for x_s, x_t, y_true in createBatches(S, T, L, self.experiment.model_kwargs['batch_size'], self.experiment.model_kwargs['rng']['shuffle']):
                        ...
                        for ip, p in enumerate(returnlevels):
                            RLmodel[i,j,ip] += returnLevelLoss(gevparams, y_true, len(self.experiment.clusters['stations']), self.experiment.model_kwargs['batch_size'], self.experiment.clusters['n'], p)
                        count += 1
                    RLmodel[i,j,:] /= count
            with open(os.path.join(self.experiment.folders['plot']['folder'], 'returnlevels.pkl'), 'wb') as f:
                pickle.dump(RLmodel, f)
            for ip, p in enumerate(returnlevels):
                Metricsevolution({f"Return level {p}":RLmodel[:,:,ip]}, self.experiment.filter['lead_times'], os.path.join(self.experiment.folders['plot']['folder'], f'ReturnLevel_{p}.png'))
            ...
            for i, params in enumerate(paramlist):
                for j, lt in enumerate(self.experiment.filter['lead_times']):
                    ...
                    for x_s, x_t, y_true in createBatches(S, T, L, self.experiment.model_kwargs['batch_size'], self.experiment.model_kwargs['rng']['shuffle']):
                        ...
                        gevparams = tmpparams if gevparams is None else jnp.concatenate([gevparams, tmpparams], axis = 0)
            Paramdistribution(gevparams, os.path.join(self.experiment.folders['plot']['folder'], 'Parameters.png'))
        
        def shapvals(self, parameter = None, cluster = None, rng = jax.random.PRNGKey(0), samplesize = 5, fold = 0):
            ...
            ioutputs = list(range(15)) if parameter is None and cluster is None else\
                [5*i + cluster for i in range(3)] if parameter is None else\
                    [iparam*5 + cluster for cluster in range(5)] if cluster is None else\
                        [iparam*5 + cluster]
            def predict(ds):
                t, s = np.split(ds, [tShape[1]], axis = -1)
                s = s.reshape(-1, sShape[1], sShape[2], sShape[3])
                return np.array(self.experiment.NN.apply(paramlist[fold], s, t)[:,ioutputs])
            explainer = shap.KernelExplainer(predict, data.mean(axis = 0, keepdims = True))
            shap_values = explainer(data[jax.random.permutation(rng, len(data))[:samplesize]])
            ...
            # Save
            with open(os.path.join(self.experiment.folders['plot']['folder'], f'shap_values_fold{fold}.pkl'), 'wb') as f:
                pickle.dump(shap_values, f)
            #Map
            ds = xr.open_dataset(self.experiment.files['test']['inputs'][0], engine = "netcdf4").isel(time = 0, lead_time = 0)
            for i in range(spatial.shape[-1]):
                ...
                extents = [5., 11.5, 45., 48.5]
                ax = fig.add_subplot(1,1,1, projection = proj)
                ax.add_feature(cfeature.BORDERS)
                ax.set_extent(extents)
                vext = max(abs(ds.shap.min()), abs(ds.shap.max()))
                cbar = ds.shap.plot(cmap = 'RdBu_r', add_colorbar = False, transform = ccrs.PlateCarree(), ax = ax, vmin = -vext, vmax = vext)
                plt.title(f"Shap values for {feature}")
                plt.colorbar(cbar, ax = ax, orientation = "vertical", label = "Shap values", shrink = 0.8)
                plt.savefig(os.path.join(self.experiment.folders['plot']['folder'], f'Shap_{feature}_{fold}.png'))
                plt.close()
            #Feature importance
            fig = plt.figure(figsize = (4.8,3.))
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(self.experiment.folders['plot']['folder'], f'Feature_importance_{fold}.png'))
            plt.close()
            return resmean
        
        def shap(self, parameter = 'mu', cluster = 0, date = "2021-06-20 17:00", lead_time = 12, rng = jax.random.PRNGKey(0), output = 'map', explainer = 'partition'):
            assert parameter in ['mu', 'sigma'], "Parameter must be either 'mu' or 'sigma'."
            iparam = 0 if parameter == 'mu' else 1
            with open(os.path.join(self.experiment.folders['scratch']['folder'], f"test_set.pkl"), 'rb') as f:
                s, t, l = pickle.load(f)
            with open(os.path.join(self.experiment.folders['plot']['folder'], 'params.pkl'), 'rb') as f:
            ...
            if date is not None:
                ...
                t_line = np.array([[[(date.day_of_year - 242)/107, # Encoding of day of year between -1 and +1
                                        np.cos(date.hour*np.pi/12),
                                        np.sin(date.hour*np.pi/12),
                                        lead_time/72]]], dtype = np.float32).reshape(-1,4)
                index = np.array([np.all(np.where(ti == t_line, True, False)) for ti in t])
            ltindex = t[:,3] == lead_time/72 if lead_time is not None else np.ones(t.shape[0], dtype = bool)
            sShape = s.shape
            tShape = t.shape
            print(sShape, tShape)
            if explainer == 'partition':
                t = np.tile(np.expand_dims(t, axis = (1,2)), [1,sShape[1],sShape[2],1])
                data = np.concatenate((t, s), axis = -1)
                def predict(ds):
                    t, s = jnp.split(ds, [tShape[1]], axis = -1)
                    t = t[:,0,0,:]
                    return np.array(self.experiment.NN.apply(paramlist[0], s, t)[:,iparam*5+cluster])
                ...
                if output == 'map':
                    if date is not None:
                        ...
                        # Open test file
                        ds = xr.open_dataset(self.experiment.files['test']['inputs'][0], engine = "netcdf4").sel(time = date, lead_time = lead_time)
                        stations = xr.open_dataset(self.experiment.files['test']['labels'][0]).isel(time = 0).sel(station = self.experiment.clusters['groups'][cluster])
                        ...
                        np.sqrt(ds.u10**2 + ds.v10**2).plot(cmap = 'viridis', add_colorbar = False, transform = ccrs.PlateCarree(), ax = ax[0])
                        plt.scatter(lons, lats, transform = ccrs.PlateCarree(), c = 'tab:orange', marker = '.', s = 1)
                        ...
                        vext = max(abs(ds.shap.min()), abs(ds.shap.max()))
                        np.sqrt(ds.u10**2 + ds.v10**2).plot(cmap = 'gray', add_colorbar = False, transform = ccrs.PlateCarree(), ax = ax[1])
                        sh = ds.shap.plot(alpha = 0.6, transform = ccrs.PlateCarree(), ax = ax[1], add_colorbar = False, cmap = 'RdBu_r', vmin = -vext, vmax = vext)
                        plt.title("Shap values")
                        cbar = plt.colorbar(sh, ax = ax, orientation = "vertical", label = "Shap values", shrink = 0.8)
                        plt.suptitle(f"Shap values for {parameter}, cluster {cluster} \nat {date} and lead time {lead_time}h")
                        plt.savefig(os.path.join(self.experiment.folders['plot']['folder'], f'Shap_{parameter}_{cluster}_{date}_{lead_time}.png'))
                        plt.close()
                        with open(os.path.join(self.experiment.folders['plot']['folder'], f'Shap_{parameter}_{cluster}_{date}_{lead_time}.pkl'), 'wb') as f:
                            pickle.dump(res, f)
                    else:
                        ...
                        vext = max(abs(ds.shap.min()), abs(ds.shap.max()))
                        cbar = ds.shap.plot(cmap = 'RdBu_r', add_colorbar = False, transform = ccrs.PlateCarree(), ax = ax, vmin = -vext, vmax = vext)
                        plt.scatter(lons, lats, transform = ccrs.PlateCarree(), c = 'tab:green', s = 5)
                        plt.title(f"Mean shap values for {parameter}, cluster {cluster}, lead time {lead_time}h")
                        plt.colorbar(cbar, ax = ax, orientation = "vertical", label = "Shap values", shrink = 0.8)
                        plt.savefig(os.path.join(self.experiment.folders['plot']['folder'], f'Shap_{parameter}_{cluster}_{lead_time}.png'))
                        plt.close()
                        with open(os.path.join(self.experiment.folders['plot']['folder'], f'Shap_{parameter}_{cluster}_{lead_time}.pkl'), 'wb') as f:
                            pickle.dump(res, f)
