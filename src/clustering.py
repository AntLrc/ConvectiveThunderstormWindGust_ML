...
class Clustering:
    def __init__(self, data, **kwargs):
        self.data = data
        self.loadAffinities(**kwargs)
        self.clusters = {}
        self.plot = self.Plotter(self)
        self.save = self.Saver(self)
        
    def loadAffinities(self, **kwargs):
        afffile = kwargs.pop("affinities", None)
        if not (afffile is None):
            try:
                with open(afffile, "rb") as f:
                    affs = pickle.load(f)
                self.affinities = affs
            except:
                return self.loadAffinities(**kwargs)
        else:
            ...
            if isinstance(var, list):
                for v in var:
                    self.affinities[v] = np.array([[xr.corr(self.data[v].isel(station=i),
                                                            self.data[v].isel(station=j), dim = "time").values
                                                    for i in range(nb_of_stations)]
                                                   for j in range(nb_of_stations)])
            else:
                self.affinities[var] = np.array([[xr.corr(self.data[var].isel(station=i),
                                                          self.data[var].isel(station=j), dim = "time").values
                                                  for i in range(nb_of_stations)]
                                                 for j in range(nb_of_stations)])
        
    def SpectralStationClustering(self, n_clusters, affinities):
        """
        ...
        clustering = SpectralClustering(n_clusters = n_clusters,
                                        ...
                                        affinity = 'precomputed').fit(affinities)
        labels = xr.DataArray(clustering.labels_, coords = {"station":self.data.station}, dims = ["station"])
        distances = np.clip(-np.log(affinities), a_min = 0, a_max = 1e8)
        np.fill_diagonal(distances, 0.)
        distances = np.nan_to_num(distances, 0.)
        self.clusters[n_clusters] = {'labels':labels, 'score':metrics.silhouette_score(distances, clustering.labels_, metric='precomputed')}
    ...
    class Plotter:
        ...
        def clusters(self, n_clusters, savepath):
            ...
            ax.scatter(self.clustering.data.longitude.isel(time = 0).values, self.clustering.data.latitude.isel(time = 0).values, c = self.clustering.clusters[n_clusters]['labels'].values, cmap = 'inferno', transform=ccrs.PlateCarree())
        ...
        def silhouette_score(self, save_path):
            ...
            ax.plot([n for n in self.clustering.clusters.keys()], [self.clustering.clusters[n]['score'] for n in self.clustering.clusters.keys()])
    ...
    class Saver:
        ...
        def csv(self, n_clusters, savepath):
            ...
            clusterlist = [list(clusterds.where(clusterds == i).dropna("station").station.values) for i in range(n_clusters)]
