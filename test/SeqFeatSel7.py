...
import pickle
expnumbers = [12, 14, 15, 17, 19, 20, 21, 22, 24, 25, 27, 28]
result = []
...
plt.title("Sequential feature selection: step 7")
plt.tight_layout()
with open("/work/FAC/FGSE/IDYST/tbeucler/downscaling/alecler1/plots/R_VGAM/Diagnostics/SeqFeatSel6.pkl", "rb") as f:
    df0 = pickle.load(f)
x = df0.where(df0['Variable'] == 'q_1000hPa')['Skill score'].dropna().mean()
values = [x]*len(order)
plt.plot(order, values, '--')
plt.savefig("/work/FAC/FGSE/IDYST/tbeucler/downscaling/alecler1/plots/R_VGAM/Diagnostics/SeqFeatSel7.png")
with open("/work/FAC/FGSE/IDYST/tbeucler/downscaling/alecler1/plots/R_VGAM/Diagnostics/SeqFeatSel7.pkl", "wb") as f:
    pickle.dump(df, f)
plt.close()
